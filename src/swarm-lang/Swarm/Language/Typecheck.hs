...
infer s@(CSyntax l t cs) = addLocToTypeErr l $ case t of
  ...
  SRcd m -> do
    m' <- traverse (itraverse $ \x -> infer . fromMaybe (RTerm (TVar (lvVar x)))) m
...
check s@(CSyntax l t cs) expected = addLocToTypeErr l $ case t of
  ...
  STydef x pty _ t1 -> do
    ...
    t1' <- withBindingTD (lvVar x) tydef (check t1 expected)
  ...
  SRcd fields
    | UTyRcd tyMap <- expected -> do
        ...
        fields' <-
          traverse
            (\(x, mt, ty) -> (x,) . Just <$> check (fromMaybe (RTerm (TVar (lvVar x))) mt) ty)
            fieldsWithTypes
  ...
  SRcd m -> sum <$> traverse analyzeField m
   where
    analyzeField ::
      ...
      m Int
    analyzeField (LV _ x, Nothing) = analyzeAtomic locals (Syntax NoLoc (TVar x) mempty ())
