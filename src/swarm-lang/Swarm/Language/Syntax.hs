...
constInfo c = case c of
  ...
  Build ->
    command 1 long . doc (Mutation $ Set.singleton $ RobotChange ExistenceChange) "Construct a new robot." $
  ...
  Salvage ->
    command 0 long . doc (Mutation $ Set.singleton $ RobotChange ExistenceChange) "Deconstruct an old robot." $
      ["Salvaging a robot will give you its inventory, equipped devices and log."]
  Say ->
    command 1 short . doc (Mutation $ Set.singleton $ RobotChange BehaviorChange) "Emit a message." $
  ...
  Listen ->
    command 1 long . doc (Query $ Sensing RobotSensing) "Listen for a message from other actors." $
  ...
  Log -> command 1 short $ shortDoc (Query $ Sensing RobotSensing) "Log the string in the robot's logger."
  View ->
    command 1 short . doc (Query $ Sensing RobotSensing) "View the given actor." $
      [ "This will recenter the map on the target robot and allow its inventory and logs to be inspected."
      ]
  Appear ->
    command 1 short . doc (Mutation $ Set.singleton Cosmetic) "Set how the robot is displayed." $
  ...
  Structure ->
    command 2 Intangible . doc (Query $ Sensing EntitySensing) "Get the x, y coordinates of the southwest corner of a constructed structure, by name and index" $
  ...
  Floorplan ->
    command 1 Intangible . doc (Query APriori) "Get the dimensions of a structure template" $
  ...
  HasTag ->
    command 2 Intangible . doc (Query APriori) "Check whether the given entity has the given tag" $
  ...
  TagMembers ->
    command 2 Intangible . doc (Query APriori) "Get the entities labeled by a tag, by alphabetical index" $
  ...
  Detect ->
    command 2 Intangible . doc (Query $ Sensing EntitySensing) "Detect an entity within a rectangle." $
  ...
  Resonate ->
    command 2 Intangible . doc (Query $ Sensing EntitySensing) "Count specific entities within a rectangle." $
  ...
  Density ->
    command 1 Intangible . doc (Query $ Sensing EntitySensing) "Count all entities within a rectangle." $
  ...
  Sniff ->
    command 1 short . doc (Query $ Sensing EntitySensing) "Determine distance to entity." $
  ...
  Chirp ->
    command 1 short . doc (Query $ Sensing EntitySensing) "Determine direction to entity." $
  ...
  Watch ->
    command 1 short . doc (Query $ Sensing EntitySensing) "Interrupt `wait` upon location changes." $
  ...
  Surveil ->
    command 1 short . doc (Query $ Sensing EntitySensing) "Interrupt `wait` upon (remote) location changes." $
      [ "Like `watch`, but with no restriction on distance."
      ]
  Heading -> command 0 Intangible $ shortDoc (Query $ Sensing RobotSensing) "Get the current heading."
  Blocked -> command 0 Intangible $ shortDoc (Query $ Sensing EntitySensing) "See if the robot can move forward."
  Scan ->
    command 1 Intangible . doc (Query $ Sensing EntitySensing) "Scan a nearby location for entities." $
  ...
  Upload -> command 1 short $ shortDoc (Mutation $ Set.singleton $ RobotChange BehaviorChange) "Upload a robot's known entities and log to another robot."
  Ishere -> command 1 Intangible $ shortDoc (Query $ Sensing EntitySensing) "See if a specific entity is in the current location."
  Isempty ->
    command 0 Intangible . doc (Query $ Sensing EntitySensing) "Check if the current location is empty." $
      [ "Detects whether or not the current location contains an entity."
      , "Does not detect robots or other actors."
      ]
  Self -> function 0 $ shortDoc (Query APriori) "Get a reference to the current robot."
  Parent -> function 0 $ shortDoc (Query APriori) "Get a reference to the robot's parent."
  Base -> function 0 $ shortDoc (Query APriori) "Get a reference to the base."
  Meet -> command 0 Intangible $ shortDoc (Query $ Sensing RobotSensing) "Get a reference to a nearby actor, if there is one."
  MeetAll -> command 0 long $ shortDoc (Mutation $ Set.singleton $ RobotChange BehaviorChange) "Run a command for each nearby actor."
  Whoami -> command 0 Intangible $ shortDoc (Query $ Sensing RobotSensing) "Get the robot's display name."
  Setname -> command 1 short $ shortDoc (Mutation $ Set.singleton $ RobotChange BehaviorChange) "Set the robot's display name."
  Random ->
    command 1 Intangible . doc Computation "Get a uniformly random integer." $
      ["The random integer will be chosen from the range 0 to n-1, exclusive of the argument."]
  Run -> command 1 long $ shortDoc (Mutation $ Set.singleton $ RobotChange BehaviorChange) "Run a program loaded from a file."
  Return -> command 1 Intangible $ shortDoc Computation "Make the value a result in `cmd`."
  Try -> command 2 Intangible $ shortDoc Computation "Execute a command, catching errors."
  Undefined -> function 0 $ shortDoc Computation "A value of any type, that is evaluated as error."
  Fail -> function 1 $ shortDoc Computation "A value of any type, that is evaluated as error with message."
  If ->
    function 3 . doc Computation "If-Then-Else function." $
      ["If the bool predicate is true then evaluate the first expression, otherwise the second."]
  Inl -> function 1 $ shortDoc Computation "Put the value into the left component of a sum type."
  Inr -> function 1 $ shortDoc Computation "Put the value into the right component of a sum type."
  Case -> function 3 $ shortDoc Computation "Evaluate one of the given functions on a value of sum type."
  Fst -> function 1 $ shortDoc Computation "Get the first value of a pair."
  Snd -> function 1 $ shortDoc Computation "Get the second value of a pair."
  Force -> function 1 $ shortDoc Computation "Force the evaluation of a delayed value."
  Not -> function 1 $ shortDoc Computation "Negate the boolean value."
  Neg -> unaryOp "-" 7 P $ shortDoc Computation "Negate the given integer value."
  Add -> binaryOp "+" 6 L $ shortDoc Computation "Add the given integer values."
  And -> binaryOp "&&" 3 R $ shortDoc Computation "Logical and (true if both values are true)."
  Or -> binaryOp "||" 2 R $ shortDoc Computation "Logical or (true if either value is true)."
  Sub -> binaryOp "-" 6 L $ shortDoc Computation "Subtract the given integer values."
  Mul -> binaryOp "*" 7 L $ shortDoc Computation "Multiply the given integer values."
  Div -> binaryOp "/" 7 L $ shortDoc Computation "Divide the left integer value by the right one, rounding down."
  Exp -> binaryOp "^" 8 R $ shortDoc Computation "Raise the left integer value to the power of the right one."
  Eq -> binaryOp "==" 4 N $ shortDoc Computation "Check that the left value is equal to the right one."
  Neq -> binaryOp "!=" 4 N $ shortDoc Computation "Check that the left value is not equal to the right one."
  Lt -> binaryOp "<" 4 N $ shortDoc Computation "Check that the left value is lesser than the right one."
  Gt -> binaryOp ">" 4 N $ shortDoc Computation "Check that the left value is greater than the right one."
  Leq -> binaryOp "<=" 4 N $ shortDoc Computation "Check that the left value is lesser or equal to the right one."
  Geq -> binaryOp ">=" 4 N $ shortDoc Computation "Check that the left value is greater or equal to the right one."
  Format -> function 1 $ shortDoc Computation "Turn an arbitrary value into a string."
  Concat -> binaryOp "++" 6 R $ shortDoc Computation "Concatenate the given strings."
  Chars -> function 1 $ shortDoc Computation "Counts the number of characters in the text."
  Split ->
    function 2 . doc Computation "Split the text into two at given position." $
  ...
  CharAt ->
    function 2 . doc Computation "Get the character at a given index." $
  ...
  ToChar ->
    function 1 . doc Computation "Create a singleton `text` value from the given character code." $
      [ "That is, `chars (toChar c) == 1` and `charAt 0 (toChar c) == c`."
      ]
  AppF ->
    binaryOp "$" 0 R . doc Computation "Apply the function on the left to the value on the right." $
      [ "This operator is useful to avoid nesting parentheses."
      , "For exaple:"
      , "`f $ g $ h x = f (g (h x))`"
      ]
  Swap ->
    command 1 short . doc (Mutation $ Set.fromList [EntityChange, RobotChange InventoryChange]) "Swap placed entity with one in inventory." $
  ...
  Atomic ->
    command 1 Intangible . doc MetaEffect "Execute a block of commands atomically." $
  ...
  Instant ->
    command 1 Intangible . doc MetaEffect "Execute a block of commands instantly." $
      [ "Like `atomic`, but with no restriction on program size."
      ]
  Key ->
    function 1 . doc Computation "Create a key value from a text description." $
  ...
  InstallKeyHandler ->
    command 2 Intangible . doc (Mutation $ Set.singleton $ RobotChange BehaviorChange) "Install a keyboard input handler." $
  ...
  Teleport -> command 2 short $ shortDoc (Mutation $ Set.singleton $ RobotChange PositionChange) "Teleport a robot to the given location."
  As -> command 2 Intangible $ shortDoc (Mutation $ Set.singleton $ RobotChange BehaviorChange) "Hypothetically run a command as if you were another robot."
  RobotNamed -> command 1 Intangible $ shortDoc (Query $ Sensing RobotSensing) "Find an actor by name."
  RobotNumbered -> command 1 Intangible $ shortDoc (Query $ Sensing RobotSensing) "Find an actor by number."
  Knows -> command 1 Intangible $ shortDoc (Query $ Sensing RobotSensing) "Check if the robot knows about an entity."
 where
