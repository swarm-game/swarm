...
loadScenarioCollection = simpleErrorHandle $ do
  ...
  -- 'loadScenarios' below, using 'evalAccum'.  Any warnings will be
  -- caught when loading all the scenarios via the usual code path; we
  -- do not need to do anything with them here while simply rendering
  -- pedagogy info.
  worlds <- evalAccum (mempty :: Seq SystemFailure) $ loadWorlds entities
  evalAccum (mempty :: Seq SystemFailure) $ loadScenarios entities worlds
