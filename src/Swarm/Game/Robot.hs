...
installedDevices :: Lens' (Robot p) Inventory
...
--   info panel.
inventoryHash :: Getter (Robot p) Int
...
systemRobot :: Lens' (Robot p) Bool

-- | Does this robot wish to self destruct?
selfDestruct :: Lens' (Robot p) Bool
...
mkRobot name l d m devs =
  Robot
    ...
    , _selfDestruct = False
    , _tickSteps = 0
    , _robotContext = emptyVarContext
    }
 where
  inst = fromList devs

-- | The initial robot representing your "base".
baseRobot :: [Entity] -> Robot 'BaseRobot
baseRobot devs =
  Robot
    { _robotEntity =
        mkEntity
