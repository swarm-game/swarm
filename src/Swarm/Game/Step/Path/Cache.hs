...
-- or from a different position than its previous invocation,
-- then the shortest-path shall be recomputed and the cache overwritten
-- with this new result.
--
-- Independent from the event of invoking the 'Path' command,
...
--
-- Some other events allow for partial re-use of the previously
-- computed path.
...
revalidatePathCache entityLoc entityModification (rid, pc) = do
  ...
  let eitherNewPathfinding = case maybeRobot of
        ...
        Just bot ->
          perhapsInvalidateForRobot
            ...
            entityModification
            pc
      (logEntry, updateFunc) = getCacheUpdate eitherNewPathfinding

  pathCaching . pathCachingRobots %= updateFunc
  pathCaching . pathCachingLog %= RB.insert (CacheLogEntry rid logEntry)
 where
  getCacheUpdate eitherNewPathfinding = case eitherNewPathfinding of
