...
pathCommand maybeLimit (Cosmic currentSubworld robotLoc) target = do
  ...
  rid <- use robotID
  let maybeCachedPath = do
        PathfindingCache t swn _ _ (TailMap ps) <- IM.lookup rid pcr
        guard $ t == target
        guard $ swn == currentSubworld
        M.lookup robotLoc ps

  case maybeCachedPath of
    Just cachedPath -> return $ Just $ nextDir cachedPath
    Nothing -> do
      ...
      runMaybeT $ do
        ...
        lift $ recordCache target currentSubworld $ robotLoc :| foundPath
