    ...
    then deleteRobot rn
      ...
      case waitingUntil rob of
        Just wakeUpTime
          ...
          | wakeUpTime <= addTo 2 time -> return ()
  ...
  case c of
    ...
    Wait -> case vs of
      [VInt d] -> do
        time <- use ticks
        purgeFarAwayWatches
        return $ Waiting (addTo d time) (Out VUnit s k)
      _ -> badConst
    Selfdestruct -> do
      ...
      [] -> do
        ...
        case mtarget of
          ...
          Just target -> do
            ...
            time <- use ticks
            return $ Waiting (addTo (fromIntegral numItems + 1) time) (Out VUnit s k)
      _ -> badConst
  ...
  finishCookingRecipe r v wf rf =
    if remTime <= 0
      then do
        ...
        time <- use ticks
        return . (if remTime <= 1 then id else Waiting (addTo remTime time)) $
