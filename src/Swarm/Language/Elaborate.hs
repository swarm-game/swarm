...
elaborate =
 ...
 where
  ...
  rewrite (SApp (SApp (SApp (TConst If) cond) thn) els) =
    SApp (TConst Force) (SApp (SApp (SApp (TConst If) cond) (TDelay thn)) (TDelay els))
  -- Rewrite any recursive occurrences of x inside t1 to (force x).
  -- When interpreting t1, we will put a binding (x |-> delay t1)
  -- in the context.
  rewrite (SLet x ty t1 t2) = SLet x ty (mapFree1 x (SApp (TConst Force)) t1) t2
  -- Rewrite any recursive occurrences of x inside t1 to (force x).
  ...
  -- bound by 'def'.
  rewrite (SDef x ty t1) = SDef x ty (mapFree1 x (SApp (TConst Force)) t1)
  ...
  rewrite (SApp (SApp (TConst Build) nm) prog) =
    SApp (SApp (TConst Build) nm) (TDelay prog)
