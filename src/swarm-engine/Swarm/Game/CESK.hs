...
data Frame
  ...
  | -- | We are in the process of evaluating an import; once done, we
    --   should proceed to evaluate the given body, in the given
    --   environment extended with the one generated by the import
    --   (restricted to the given set of names that are supposed to be
    --   exported from the module; see Note [Module exports]).
    --
    --   The first field is the location of the import; the second
    --   field records the names which are supposed to be exported
    --   from the module; the third field records the term we should
...
prettyFrame f (p, inner) = case f of
  ...
  FImport _ _ t _ -> (11, hsep ["import", inner, "in", ppr t])
