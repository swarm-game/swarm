...
main = do
  ...
  solutionTests <- testScenarioSolutions $ PersistentState rs' ui key progState
  defaultMain $
    testGroup
      "Tests"
...
data TestType
  ...
  |
    Timed Time
  |
    Special Time ShouldCheckBadErrors (GameState -> Assertion)
  |
    DefaultTutorial
  |
    TimedTutorial Time
  |
    SpecialTutorial Time ShouldCheckBadErrors (GameState -> Assertion)

data ScenarioTestData = ScenarioTestData
  Time
  FilePath
...
test file DefaultTest = singletonTest $ ScenarioTestData Default file CheckForBadErrors (const $ pure ())
test file (Timed timed) = singletonTest $ ScenarioTestData timed file CheckForBadErrors (const $ pure ())
test file (Special timed shouldCheckBadErrors verify) = singletonTest $ ScenarioTestData timed file shouldCheckBadErrors verify
test file DefaultTutorial = singletonTest $ ScenarioTestData Default file CheckForBadErrors tutorialHasLog
test file (TimedTutorial timed) = singletonTest $ ScenarioTestData timed file CheckForBadErrors tutorialHasLog
test file (SpecialTutorial timed shouldCheckBadErrors verify) = singletonTest $ ScenarioTestData timed file shouldCheckBadErrors (\g -> tutorialHasLog g >> verify g)
...
instance Semigroup TestData where
  (<>) (TestData a) (TestData b) = TestData (b `S.union` a)

discoverScenarios :: IO TestData
discoverScenarios = do
  scenarioList <- findAllWithExt "data/scenarios" "yaml"
  pure . sconcat . N.fromList . map (`test` DefaultTest) $ scenarioList
