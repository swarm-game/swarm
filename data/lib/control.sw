...
def liftA2: (a -> b -> c) -> Cmd a -> Cmd b -> Cmd c
  = \f. \ca. \cb.
  a <- ca;
  b <- cb;
  pure (f a b)
...
def notC: Cmd Bool -> Cmd Bool = \c. b <- c; pure (not b) end
...
def orC: Cmd Bool -> Cmd Bool -> Cmd Bool = \c1. \c2. ifC c1 {pure true} {c2}
end
...
def when: Cmd Bool -> Cmd a -> Cmd Unit
  = \test. \cmd.
  b <- test;
  if b {cmd; pure ()} {}
end
